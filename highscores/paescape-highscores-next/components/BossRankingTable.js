...
const BossRankingTable = ({ data }) => {
  let boss = "";
  let img = "blank.gif";
  if (data.boss !== undefined) {
    boss = data.boss;
    img = `game_icon_${constants.BOSSES[boss]
      .toLowerCase()
      .replace(/ /g, "")
      .replace(/\(/g, "")
      .replace(/\)/g, "")
    ...
    <div className={styles.contentHiscores}>
      <table>
        <caption>
          <Image
            ...
            alt=""
          />
          {constants.BOSSES[boss]} Hiscores
        </caption>
        <thead>
          <tr>
            ...
            return (
              <tr className={rowStyles} key={player.username}>
                ...
                <td className={styles.right}>
                  {utils.numberWithCommas(player.kills)}
                </td>
              </tr>
            );
          })}
      ...
      <nav className={styles["personal-hiscores__pagination"]}>
        {data.prevPage !== 0 && (
          <Link
            href={{
              pathname: `/boss/${boss.toLowerCase()}`,
...
export default BossRankingTable;
